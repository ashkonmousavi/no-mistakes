...
The `review`, `test`, and configured-command `lint` steps use this shared model directly. The `document` step also uses the same `action` field, but adds a required class that decides whether a finding gates at all; the [Document step](/no-mistakes/reference/pipeline-steps/#document) owns those rules.
