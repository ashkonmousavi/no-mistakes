...
The trusted `sync_strategy` key (default `rebase`) selects how this integration happens: `rebase` rewrites the branch's history as described below, while `sync_strategy: merge` integrates with an ordinary `git merge` instead - a merge commit, or a fast-forward when possible, so the pushed head always stays an ancestor of the new head and history is never rewritten. `sync_strategy: merge` also applies to the CI step's conflict-repair agent (see [CI](#ci) below), so a repository that forbids rebasing never has one run through either path.
...
- If a fast-forward is possible, moves the branch forward without a rebase or merge commit (a hard-reset under `sync_strategy: rebase`, `git merge --ff-only` under `sync_strategy: merge` - both are pure fast-forwards, never a history rewrite)
- If the diff against the PR base branch is empty after rebase, completes rebase and skips all remaining pipeline steps
- On conflict: records conflicting files, aborts the rebase (or merge, under `sync_strategy: merge`), and reports findings
...
| `behavioural` | A substantive defect in a file that influences executable behaviour, generated output, or delivery authority - a specification a validator or generator reads, a contract or records file a check verifies, a policy file that steers a gate - whatever its extension, `.md` included | Blocks |

The `behavioural` class is why a correction owes the project's own [`commands.test`](/no-mistakes/reference/repo-config/#commandstest) at all, but the rerun is not branched on a finding's class. **Every** correction re-runs it, because the run continues from Test whenever the head advance is inside the documentation-and-records path class - see "Head handling after a correction" below. A `substantive`-only correction is therefore re-validated exactly as thoroughly as a `behavioural` one.
...
- On GitHub, GitLab, Forgejo, or Azure DevOps merge conflict: asks the agent to integrate the latest PR base branch tip (rebase by default, or an ordinary merge under [`sync_strategy: merge`](/no-mistakes/reference/repo-config/#sync_strategy), which also forbids the agent from running `git rebase`, a `git reset --hard` onto another commit, or a force-push) and make the smallest correct root-cause fix for the conflicts, using user intent when available
