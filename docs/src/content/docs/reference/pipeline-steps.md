...
[`commands.test`](/no-mistakes/reference/repo-config/#commandstest) owns the configuration contract for any explicit baseline command, and [`test_command_sufficient`](/no-mistakes/reference/repo-config/#test_command_sufficient) owns whether a passing command retires the evidence pass.
