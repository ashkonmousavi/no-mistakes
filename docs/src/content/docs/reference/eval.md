...
Automatic collection also runs the false-negative path for a defect CI surfaced. When a finished run's CI step reports a real code finding - a failing check the provider attributes to the job (`ci-check`) or a review-bot finding (`ci-review-bot`), which carries an unresolved comment when available or the red check itself as a fallback - that auto-fix or the user selected, the following fix round records a published repair, and post-repair checks pass, that finding is a Review false negative only for the exact green Review epoch whose head CI observed. It is ingested as false-negative gold (`recorded-ci-false-negative`) onto that Review round's case, reading only the structured finding the CI step already persisted per round (its category, check identity, and any file/line the finding recorded). A published repair, documentation-authority carry, or another post-review head mutation ends eligibility until a later Review passes green on the new head, so a defect introduced after an unreviewed mutation is excluded rather than attributed backwards. The path never enriches from log text, never fabricates a location the finding did not carry, and makes no cross-run judgement. Also excluded are `ci-transient` and other provider/infrastructure failures (no code change clears them), merge conflicts, repairs that produced or published no change, no-CI declarations, terminal PR completion before checks pass, and any finding the human dismissed at the gate rather than fixing. Re-ingesting the same run is a no-op, and the final automatic retention pass includes any epoch cases ingestion recaptured. Like automatic review-case collection, this requires `eval.auto_capture` and `eval.capture_provenance`.
...
- A CI finding (`ci-check` or `ci-review-bot`) observed on an exact green-Review epoch, selected by auto-fix or the user, published by the following fix round, and cleared by post-repair checks is **false-negative** gold (`recorded-ci-false-negative`) on that Review round's case, ingested automatically when the run finishes. A later unreviewed head mutation ends eligibility; `ci-transient`/provider-infra failures, merge conflicts, and dismissed findings are also excluded.
